...
/**
 * Match a paid Shopier order back to a draft (dynamic product per checkout).
 */
async function resolveDraftFromPayment(order, store) {
  ...
  for (const item of order?.lineItems || []) {
    if (item?.productId && store.findByShopierProductId) {
      const byProduct = await store.findByShopierProductId(String(item.productId));
      if (byProduct) return byProduct.id;
    }
  }

  const email = String(
    order?.billingInfo?.email || order?.shippingInfo?.email || ''
  )
    .trim()
    .toLowerCase();
...
async function createPaymentRequest({ draftId }) {
  ...
  await purgeOtherListingProducts(productId);

  console.info('[shopier] opening checkout for shared product', {
    draftId,
    productId,
    shopSlug: config.shopSlug,
  });
  ...
  try {
    ...
    return {
      checkoutUrl: `https://www.shopier.com/${config.shopSlug}/${productId}`,
      checkoutHtml: wrapCheckoutHtml(checkoutHtml),
      productId,
      draftId,
    };
  } catch (err) {
...
async function findPaidOrderForDraft(draft) {
  ...
  try {
    if (draft.shopierProductId) {
      const orders = normalizeOrderList(
        await client.orders.list({
          productId: String(draft.shopierProductId),
          limit: 10,
        })
      );
      const paid = orders.find((order) => order.paymentStatus === 'paid');
    ...
    if (draft.buyerEmail) {
      const orders = normalizeOrderList(
        await client.orders.list({
          customerEmail: String(draft.buyerEmail).trim(),
          limit: 10,
        })
      );
      const paid = orders.find((order) => order.paymentStatus === 'paid');
