...
const APP_PRODUCT_PATTERN =
  /love letter|valentine|surprise|personalized romantic|v2026-/i;

function isAppCheckoutProduct(product) {
  if (!product) return false;
  if (product.customListing) return true;
  const blob = [
    product.title,
    product.description,
    product.customNote,
...
async function listCheckoutListingProducts() {
  ...
  return products.filter(isAppCheckoutProduct);
}
