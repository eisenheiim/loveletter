...
function buildCheckoutRedirectPage(checkoutUrl, priceLabel = '€1.00') {
...
<body style="margin:0;min-height:100dvh;display:flex;align-items:center;justify-content:center;padding:2rem;background:linear-gradient(180deg,#fff5f7,#ffe8ec);font-family:system-ui,sans-serif;text-align:center;">
  <div>
    <p style="margin:0 0 0.5rem;font-size:0.75rem;letter-spacing:0.2em;text-transform:uppercase;color:#9b1b30;">Payment total</p>
    <p style="margin:0 0 0.75rem;font-size:3rem;font-weight:600;color:#7f0a1e;">${priceLabel}</p>
    <p style="margin:0;font-size:0.95rem;color:#9b1b30;">Opening secure payment&hellip;</p>
  </div>
  <script>
    setTimeout(function () {
...
async function purgeAllDigitalProducts() {
  if (!isPaymentConfigured()) return { total: 0, deleted: 0, failed: 0 };
  const client = getApiClient();
  ...
  const results = await Promise.allSettled(
    products.map((product) => deleteShopierProduct(product.id))
  );
...
/**
 * Create a Shopier PAT checkout for a draft.
 * Uses direct product URL (not /s/shipping/) so old shop products do not appear in summary.
 */
async function createPaymentRequest({ draftId }) {
  const config = getConfig();
  const flow = getPaymentFlow();

  await purgeAllDigitalProducts();
  ...
  try {
    ...
    const checkoutUrl = payment.paymentUrl;
    return {
      checkoutUrl,
      checkoutHtml: buildCheckoutRedirectPage(checkoutUrl),
...
async function listCheckoutListingProducts() {
...
async function deleteAllCheckoutListingProducts() {
  return purgeAllDigitalProducts();
}
...
module.exports = {
  ...
  getPat,
  getShopSlug,
  listCheckoutListingProducts,
  deleteShopierProduct,
  deleteAllCheckoutListingProducts,
  purgeAllDigitalProducts,
  testShopierConnection,
  extractShopierErrorMessage,
  wrapCheckoutHtml,
  buildCheckoutRedirectPage,
};
