...
const { getCheckoutChargeAmount, getCheckoutChargeCurrency } = require('./checkoutCharge');
...
async function testShopierConnection() {
  ...
  try {
    ...
    let sharedProductId = getSharedProductIdSync();
    if (!sharedProductId) {
      sharedProductId = await loadPersistedSharedProductId();
    }
    return {
      ok: true,
      ...
      sharedProductId: sharedProductId || null,
    };
  } catch (err) {
    return {
      ok: false,
...
let cachedSharedProductId = null;

const SHARED_PRODUCT_ID_FILE = path.join(__dirname, '..', 'data', 'shopier-product-id.json');
const SHARED_PRODUCT_TITLE = 'Personalized Love Letter Webpage';

function formatProductPrice(amount) {
  const raw = typeof amount === 'number' ? amount : Number(String(amount).trim().replace(',', '.'));
  if (!Number.isFinite(raw) || raw <= 0) {
    throw new Error('PRODUCT_PRICE must be a positive number');
  }
  return raw.toFixed(2);
}

async function loadPersistedSharedProductId() {
  try {
    const raw = await fs.readFile(SHARED_PRODUCT_ID_FILE, 'utf8');
    const data = JSON.parse(raw);
    return data.productId ? String(data.productId).trim() : null;
  } catch {
    return null;
  }
}

async function persistSharedProductId(productId) {
  await fs.mkdir(path.dirname(SHARED_PRODUCT_ID_FILE), { recursive: true });
  await fs.writeFile(
    SHARED_PRODUCT_ID_FILE,
    JSON.stringify({ productId: String(productId) }, null, 2),
    'utf8'
  );
}

/**
 * One Shopier product for every checkout. Set SHOPIER_PRODUCT_ID to an existing
 * product from your Shopier panel, or let the server create it once on first pay.
 */
async function ensureSharedProduct() {
  if (cachedSharedProductId) return cachedSharedProductId;

  const fromEnv = (process.env.SHOPIER_PRODUCT_ID || '').trim();
  if (fromEnv) {
    cachedSharedProductId = fromEnv;
    return fromEnv;
  }

  const fromFile = await loadPersistedSharedProductId();
  if (fromFile) {
    cachedSharedProductId = fromFile;
    return fromFile;
  }

  if (process.env.NODE_ENV === 'production') {
    throw new Error(
      'SHOPIER_PRODUCT_ID is required in production. Create one product in your Shopier panel and add its id to Render.'
    );
  }

  if (!isPaymentConfigured()) {
    throw new Error('Shopier is not configured');
  }

  const config = getConfig();
  const client = getApiClient();

  console.info('[shopier] creating shared catalog product (one-time)');

  const product = await client.products.create({
    title: SHARED_PRODUCT_TITLE,
    description: 'Custom romantic surprise webpage',
    type: 'digital',
    shippingPayer: 'sellerPays',
    priceData: {
      currency: config.currency,
      price: formatProductPrice(config.price),
    },
    media: [
      {
        type: 'image',
  ...
  if (!product?.id) {
    throw new Error('Shopier did not return a product id for the shared catalog product');
  }

  cachedSharedProductId = String(product.id);
  await persistSharedProductId(cachedSharedProductId);

  console.warn(
    `[shopier] Shared product created: ${cachedSharedProductId}. ` +
      `Set SHOPIER_PRODUCT_ID=${cachedSharedProductId} in Render and delete old test products in Shopier.`
  );

  return cachedSharedProductId;
}

function isSharedShopierProductId(productId) {
  const sharedId = getSharedProductIdSync();
  return Boolean(sharedId && productId && String(productId) === String(sharedId));
}

function getOrderEmail(order) {
  return String(order?.billingInfo?.email || order?.shippingInfo?.email || order?.customerEmail || '')
    .trim()
    .toLowerCase();
}

function getOrderCreatedTime(order) {
  const raw = order?.createdAt || order?.created_at || order?.date || order?.orderedAt;
  const time = raw ? new Date(raw).getTime() : NaN;
  return Number.isFinite(time) ? time : 0;
}

/**
 * Match a paid Shopier order back to a draft (single shared catalog product).
 */
async function resolveDraftFromPayment(order, store) {
  let draftId = extractDraftIdFromOrder(order);
  if (draftId) return draftId;

  for (const item of order?.lineItems || []) {
    if (item?.productId && store.findByShopierProductId) {
      const productId = String(item.productId);
      if (isSharedShopierProductId(productId)) continue;
  ...
  if (email && store.findUnpaidByBuyerEmail) {
    const byEmail = await store.findUnpaidByBuyerEmail(email);
    if (byEmail) return byEmail.id;
  }

  if (store.findMostRecentPendingCheckout) {
    const pending = await store.findMostRecentPendingCheckout({
...
function getSharedProductIdSync() {
  return (
    cachedSharedProductId ||
    (process.env.SHOPIER_PRODUCT_ID || '').trim() ||
    null
  );
}
...
function getConfig() {
  ...
  return {
    ...
    price: getCheckoutChargeAmount(),
    currency: getCheckoutChargeCurrency(),
    baseUrl,
    productImageUrl,
  };
}
...
/**
 * Open hosted checkout for the single shared Shopier product (no new product per order).
 */
async function createPaymentRequest({ draftId }) {
  const config = getConfig();
  const productId = await ensureSharedProduct();

  console.info('[shopier] opening shared-product checkout', {
    draftId,
    productId,
    shopSlug: config.shopSlug,
    amount: config.price,
  ...
  try {
    ...
    const checkoutUrl = `https://www.shopier.com/${config.shopSlug}/${productId}`;
    const wrappedHtml = wrapCheckoutHtml(checkoutHtml);

    return {
      draftId,
      productId,
      checkoutUrl,
      checkoutHtml: wrappedHtml,
      payment_url: checkoutUrl,
      checkout_html: wrappedHtml,
...
/**
 * Fallback when webhook is missing: look up a paid Shopier order for this draft.
 */
async function findPaidOrderForDraft(draft) {
  if (!draft || draft.isPaid || !isPaymentConfigured()) return null;

  const client = getApiClient();
  const checkoutStarted = draft.checkoutStartedAt
    ? new Date(draft.checkoutStartedAt).getTime()
    : 0;
  const buyerEmail = String(draft.buyerEmail || '').trim().toLowerCase();

  function matchesDraftWindow(order) {
    if (!checkoutStarted) return true;
    const orderTime = getOrderCreatedTime(order);
    if (!orderTime) return false;
    return orderTime >= checkoutStarted - 60 * 1000;
  }
  ...
  try {
    ...
    if (!orders.length && draft.shopierProductId) {
      orders = normalizeOrderList(
        await client.orders.list({
          productId: String(draft.shopierProductId),
          limit: 20,
        })
      );
      if (buyerEmail && isSharedShopierProductId(draft.shopierProductId)) {
        orders = orders.filter((order) => getOrderEmail(order) === buyerEmail);
      }
    }

    const paid = orders
      .filter((order) => isPaidOrder(order) && matchesDraftWindow(order))
      .sort((a, b) => getOrderCreatedTime(b) - getOrderCreatedTime(a))[0];

    if (paid?.id) return { orderId: paid.id, order: paid };
  } catch (err) {
    console.warn('[shopier] findPaidOrderForDraft failed', err.message);
  }

  return null;
}

const APP_PRODUCT_PATTERN =
  /premium valentine|personalized love letter|love letter|valentine surprise|v2026-/i;

function isLegacyAppProduct(product) {
  if (!product) return false;
  const blob = [product.title, product.description, product.customNote]
    .filter(Boolean)
    .join(' ');
  return APP_PRODUCT_PATTERN.test(blob);
}

/**
 * Remove old test products from Shopier so checkout summary shows one item only.
 */
async function purgeOtherListingProducts(keepProductId) {
  const keep = String(keepProductId || '').trim();
  if (!keep || !isPaymentConfigured()) {
    return { deleted: 0, kept: keep || null };
  }
  ...
  try {
    ...
    for (const product of products) {
      if (!product?.id || String(product.id) === keep) continue;
      if (!isLegacyAppProduct(product)) continue;

      const removed = await deleteShopierProduct(product.id);
      if (removed) {
        deleted += 1;
        console.info('[shopier] removed old product', product.id, product.title);
      }
    }
  } catch (err) {
    console.warn('[shopier] product cleanup failed', err.message);
  }

  if (deleted > 0) {
    console.info(`[shopier] checkout cleanup: deleted ${deleted} old product(s), kept ${keep}`);
  }
...
module.exports = {
  ...
  ensureSharedProduct,
  getSharedProductIdSync,
  isSharedShopierProductId,
  purgeOtherListingProducts,
  isConfigured,
  isPaymentConfigured,
  isWebhookConfigured,
  getConfig,
